...
class LineTalk(object):
    ...
    @loggedIn
    def nadyatj(self, img, text, stext):
    ...
    @loggedIn
    def sendImage2(self, to, path):
        contentFooter = {
           'AGENT_ICON': "http://dl.profile.line-cdn.net/" + self.getProfile().picturePath,
           'AGENT_NAME': 'Click Me!',
           'AGENT_LINK': 'line://ti/p/~nadya_tj'
           }
        objectId = self.sendMessage(to=to, text=None, contentMetadata=contentFooter, contentType=1).id
        return self.uploadObjTalk(path=path, type='image', returnAs='bool', objId=objectId)
    ...
    def sendImageWithURL2(self, to, url):
        path = self.downloadFileURL(url, 'path')
        return self.sendImage2(to, path)

    @loggedIn
    def sendImage3(self, to, path, name, link):
        contentFooter = {
            'AGENT_ICON': "http://dl.profile.line-cdn.net/" + self.getProfile().picturePath,
            'AGENT_NAME': name,
            'AGENT_LINK': link
    ...
    @loggedIn
    def sendImageWithURL3(self, to, url, name, link):
        path = self.downloadFileURL(url, 'path')
        return self.sendImage3(to, path, name, link)

    @loggedIn
    def sendImage4(self, to, path):
        objectId = self.sendMessage(to=to, text=None, contentType = 1).id
        return self.uploadObjTalk(path=path, type='image', returnAs='bool', objId=objectId)

    @loggedIn
    def sendImageWithURL4(self, to, url):
        path = self.downloadFileURL(url, 'path')
        return self.sendImage4(to, path)
